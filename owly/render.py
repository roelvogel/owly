...
def _article_count_label(count: int) -> str:
    noun = "article" if count == 1 else "articles"
    return f"{count} {noun} from RSS and X."
...
def render_main_edition(
    ...
    lines = [
        ...
        "",
        f"Generated at {now.strftime('%H:%M')}. {_article_count_label(len(result.items))}",
        "",
        "## In this edition",
        "",
    ]
    ...
    for idx, item in enumerate(result.items, start=1):
        lines.append(f"{idx}. {_clean_text(item.title)}")
